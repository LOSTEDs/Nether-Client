...
public class MapGenStronghold extends MapGenStructure {
    ...
    public MapGenStronghold() {
        ...
        for (i = (arrayOfBiomeGenBase = BiomeGenBase.getBiomeGenArray()).length, b = 0; b < i; ) {
            BiomeGenBase biomegenbase = arrayOfBiomeGenBase[b];
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        if (!this.ranBiomeCheck) {
            ...
            for (int j = 0; j < this.structureCoords.length; j++) {
                double d1 = (1.25D * k + random.nextDouble()) * this.field_82671_h * k;
                int m = (int)Math.round(Math.cos(d0) * d1);
                int l = (int)Math.round(Math.sin(d0) * d1);
                BlockPos blockpos = this.worldObj.getWorldChunkManager().findBiomePosition((m << 4) + 8, (l << 4) + 8, 112, this.field_151546_e, random);
                if (blockpos != null) {
                    m = blockpos.getX() >> 4;
                    l = blockpos.getZ() >> 4;
                } 
                this.structureCoords[j] = new ChunkCoordIntPair(m, l);
                d0 += 6.283185307179586D * k / this.field_82672_i;
        ...
        for (i = (arrayOfChunkCoordIntPair = this.structureCoords).length, b = 0; b < i; ) {
            ChunkCoordIntPair chunkcoordintpair = arrayOfChunkCoordIntPair[b];
