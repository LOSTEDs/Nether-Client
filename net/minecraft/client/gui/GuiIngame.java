...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        if (Config.isVignetteEnabled()) {
            renderVignette(Minecraft.thePlayer.getBrightness(partialTicks), scaledresolution);
        } else {
        ...
        if (!Minecraft.thePlayer.isPotionActive(Potion.confusion)) {
            float f = Minecraft.thePlayer.prevTimeInPortal + (Minecraft.thePlayer.timeInPortal - Minecraft.thePlayer.prevTimeInPortal) * partialTicks;
            if (f > 0.0F)
        ...
        if (Minecraft.thePlayer.getSleepTimer() > 0) {
            ...
            int l = Minecraft.thePlayer.getSleepTimer();
            float f2 = l / 100.0F;
            if (f2 > 1.0F)
                f2 = 1.0F - (l - 100) / 10.0F; 
        ...
        if (!this.mc.gameSettings.keyBindPlayerList.isKeyDown() || (this.mc.isIntegratedServerRunning() && Minecraft.thePlayer.sendQueue.getPlayerInfoMap().size() <= 1 && scoreobjective1 == null)) {
            this.overlayPlayerList.updatePlayerList(false);
        } else {
            this.overlayPlayerList.updatePlayerList(true);
            this.overlayPlayerList.renderPlayerlist(i, scoreboard, scoreobjective1);
        } 
        if (!(this.mc.currentScreen instanceof client.hud.HUDConfigScreen)) {
            if (this.mc.gameSettings.showDebugInfo)
                return; 
    ...
    public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
        ...
        if (i > 0) {
            short short1 = 182;
            int k = (int)(Minecraft.thePlayer.experience * (short1 + 1));
        ...
        if (Minecraft.thePlayer.experienceLevel > 0) {
            ...
            int j = Minecraft.thePlayer.experienceLevel;
            int i1 = (p_175176_1_.getScaledWidth() - getFontRenderer().getStringWidth(j)) / 2;
            int l = p_175176_1_.getScaledHeight() - 31 - 4;
            boolean flag = false;
            getFontRenderer().drawString(j, i1 + 1, l, 0);
            getFontRenderer().drawString(j, i1 - 1, l, 0);
            getFontRenderer().drawString(j, i1, l + 1, 0);
            getFontRenderer().drawString(j, i1, l - 1, 0);
            getFontRenderer().drawString(j, i1, l, j1);
            this.mc.mcProfiler.endSection();
        } 
    }
    ...
    private void renderScoreboard(ScoreObjective p_180475_1_, ScaledResolution p_180475_2_) {
        ...
        for (Score score : arraylist1) {
            ...
            String s = String.valueOf(ScorePlayerTeam.formatPlayerName((Team)scoreplayerteam, score.getPlayerName())) + ": " + EnumChatFormatting.RED + score.getScorePoints();
    ...
    private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
        if (!Config.isVignetteEnabled()) {
            ...
            float f = (float)worldborder.getClosestDistance((Entity)Minecraft.thePlayer);
