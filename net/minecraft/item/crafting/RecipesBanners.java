...
public class RecipesBanners {
    ...
    static class RecipeAddPattern implements IRecipe {
        ...
        private TileEntityBanner.EnumBannerPattern func_179533_c(InventoryCrafting p_179533_1_) {
            ...
            for (i = (arrayOfEnumBannerPattern = TileEntityBanner.EnumBannerPattern.values()).length, b = 0; b < i; ) {
                TileEntityBanner.EnumBannerPattern tileentitybanner$enumbannerpattern = arrayOfEnumBannerPattern[b];
                if (tileentitybanner$enumbannerpattern.hasValidCrafting()) {
                    boolean flag = true;
                    if (tileentitybanner$enumbannerpattern.hasCraftingStack()) {
                        boolean flag1 = false;
                        boolean flag2 = false;
                        for (int j = 0; j < p_179533_1_.getSizeInventory() && flag; j++) {
                            ItemStack itemstack = p_179533_1_.getStackInSlot(j);
                            if (itemstack != null && itemstack.getItem() != Items.banner)
