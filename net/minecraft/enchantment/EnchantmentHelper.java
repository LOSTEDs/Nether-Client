...
public class EnchantmentHelper {
    ...
    public static int getMaxEnchantmentLevel(int enchID, ItemStack[] stacks) {
        ...
        for (j = (arrayOfItemStack = stacks).length, b = 0; b < j; ) {
            ItemStack itemstack = arrayOfItemStack[b];
            int k = getEnchantmentLevel(enchID, itemstack);
            if (k > i)
                i = k; 
            b++;
        } 
    ...
    private static void applyEnchantmentModifierArray(IModifier modifier, ItemStack[] stacks) {
        ...
        int i;
        ItemStack[] arrayOfItemStack;
        for (i = (arrayOfItemStack = stacks).length, b = 0; b < i; ) {
            ItemStack itemstack = arrayOfItemStack[b];
    ...
    public static ItemStack getEnchantedItem(Enchantment p_92099_0_, EntityLivingBase p_92099_1_) {
        ...
        int i;
        ItemStack[] arrayOfItemStack;
        for (i = (arrayOfItemStack = p_92099_1_.getInventory()).length, b = 0; b < i; ) {
            ItemStack itemstack = arrayOfItemStack[b];
    ...
    public static Map<Integer, EnchantmentData> mapEnchantmentData(int p_77505_0_, ItemStack p_77505_1_) {
        ...
        for (i = (arrayOfEnchantment = Enchantment.enchantmentsBookList).length, b = 0; b < i; ) {
            ...
            if (enchantment != null && (enchantment.type.canEnchantItem(item) || flag))
                for (int j = enchantment.getMinLevel(); j <= enchantment.getMaxLevel(); j++) {
                    if (p_77505_0_ >= enchantment.getMinEnchantability(j) && p_77505_0_ <= enchantment.getMaxEnchantability(j)) {
                        if (map == null)
                            map = Maps.newHashMap(); 
                        map.put(Integer.valueOf(enchantment.effectId), new EnchantmentData(enchantment, j));
                    } 
                }  
            b++;
        } 
