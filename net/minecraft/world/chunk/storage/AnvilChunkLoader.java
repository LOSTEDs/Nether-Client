...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO {
    ...
    private void writeChunkToNBT(Chunk chunkIn, World worldIn, NBTTagCompound p_75820_3_) {
        ...
        for (i = (arrayOfExtendedBlockStorage1 = aextendedblockstorage).length, b = 0; b < i; ) {
            ExtendedBlockStorage extendedblockstorage = arrayOfExtendedBlockStorage1[b];
            if (extendedblockstorage != null) {
                ...
                for (int j = 0; j < (extendedblockstorage.getData()).length; j++) {
                    char c0 = extendedblockstorage.getData()[j];
                    int m = j & 0xF;
                    int k = j >> 8 & 0xF;
                    int l = j >> 4 & 0xF;
                    if (c0 >> 12 != 0) {
                        ...
                        nibblearray1.set(m, k, l, c0 >> 12);
                    } 
                    abyte[j] = (byte)(c0 >> 4 & 0xFF);
                    nibblearray.set(m, k, l, c0 & 0xF);
                } 
